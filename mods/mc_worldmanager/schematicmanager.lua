...
function schematicManager.getSchematic(key)
    ...
    local config = { author = _author, name = _name, format = _format, spawnPoint = _spawnPoint, schematicSize = _schematicSize,
                     ...
                     onSchematicPlaceFunction = realm_create_function_name, onRealmDeleteFunction = realm_delete_function_name, utmInfo = _utmInfo, miscData = _miscData, startOffset = _startOffset }
...
local core_files = minetest.get_dir_list(mc_core.path.."\\maps\\schematics\\", false)
local realm_files = minetest.get_dir_list(minetest.get_worldpath().."\\realmSchematics\\", false)
for k, fileName in pairs(core_files) do
    local filePath = mc_core.path.."\\maps\\schematics\\"..fileName
...
for k, fileName in pairs(realm_files) do
