...
function mc_toolhandler.register_tool_manager(tool, options)
    ...
    minetest.register_on_joinplayer(function(player)
        ...
        elseif not mc_helpers.checkPrivs(player, options.privs) then
            ...
            if player_has_multiple_copies(player, ItemStack(tool)) then
    ...
    minetest.register_on_priv_grant(function(name, granter, priv)
        ...
        if mc_helpers.checkPrivs(player, options.privs) then
            ...
            elseif player_has_multiple_copies(player, ItemStack(tool)) then
...
minetest.register_on_chatcommand(function(name, command, params)
    if command == "give" or command == "giveme" then
        ...
        local function fail_and_send_item_privs(name, stack)
            local stack_privs = mc_toolhandler.reg_tools[stack:get_name()]["privs"] or {teacher = true}
            local keys = {}
